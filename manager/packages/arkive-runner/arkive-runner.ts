...
export class ArkiveRunner implements ArkiveActor {
		...
		worker.onmessage = async (e: MessageEvent<ArkiveMessageEvent>) => {
			if (e.data.topic === 'handlerError') {
				...
				try {
					...
					logger('arkiveRunner').error(error, {
	...
	removeDeployment(deployment: { arkive: Arkive; worker: Worker }) {
		logger('arkiveRunner').info('Removing deployment', deployment)
